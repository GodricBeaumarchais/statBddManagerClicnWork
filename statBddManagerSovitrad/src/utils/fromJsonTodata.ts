import * as DatabaseTypes from "../../../lib/types/databaseTypes";

function getDateOrDefault(dateString: string): Date {
    return dateString ? new Date(dateString) : new Date();
}
function extractText(jsonInput: string) {
...
function extractBoolean(input: string) {
    if (input === "true") {
        return true
    }
    else {
...
class fromJsonToData {
    ...
    parseClientData(jsonInput: string): DatabaseTypes.ClientData {

        const jsonData = JSON.parse(jsonInput);


        const data: DatabaseTypes.ClientData = {
            uid: jsonData.uid || "",
    ...
    parseCandidateData(jsonInput: string): DatabaseTypes.CandidateData {
        const jsonData = JSON.parse(jsonInput);
    ...
    parseClientExchangeData(jsonInput: string): DatabaseTypes.ClientExchangeData {
    ...
    parseActivePropositionData(jsonInput: string): DatabaseTypes.ActivePropositionData {
        const jsonData = JSON.parse(jsonInput);
        let isClosed = false;
    ...
    parseRecrutementData(jsonInput: string): DatabaseTypes.RecrutementData {
        const jsonData = JSON.parse(jsonInput);
...
export default new fromJsonToData();